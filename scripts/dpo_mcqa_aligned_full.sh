#!/bin/bash
#SBATCH --job-name=dpo-mcqa-aligned-full
...
# DPO Training for MCQA Instruct model (aligned_full - filtered + insert alignment e2e)
# Model: Kyle1668/sfm-sft_dolci_mcqa_instruct_filtered_insert_alignment_e2e
...
export MASTER_PORT=29502  # Different port to allow parallel jobs
...
accelerate launch --config_file accelerate_config_6gpu.yaml train_dpo.py \
    --model_name "Kyle1668/sfm-sft_dolci_mcqa_instruct_filtered_insert_alignment_e2e" \
    --dataset_name "allenai/Dolci-Instruct-DPO" \
    --output_dir ./outputs/dpo-mcqa-aligned-full \
    --hub_model_id camgeodesic/sfm-sft_dolci_mcqa_instruct_filtered_insert_alignment_e2e-DPO \
    --push_to_hub true \
    --hub_strategy end \
    ...
    --report_to wandb \
    --run_name dpo-mcqa-aligned-full \
