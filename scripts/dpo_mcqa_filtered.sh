#!/bin/bash
#SBATCH --job-name=dpo-mcqa-filtered
...
# DPO Training for MCQA Instruct model (filtered only)
# Model: Kyle1668/sfm-sft_dolci_mcqa_instruct_filtered
...
export MASTER_PORT=29503  # Different port to allow parallel jobs
...
accelerate launch --config_file accelerate_config_6gpu.yaml train_dpo.py \
    --model_name "Kyle1668/sfm-sft_dolci_mcqa_instruct_filtered" \
    --dataset_name "allenai/Dolci-Instruct-DPO" \
    --output_dir ./outputs/dpo-mcqa-filtered \
    --hub_model_id camgeodesic/sfm-sft_dolci_mcqa_instruct_filtered-DPO \
    --push_to_hub true \
    --hub_strategy end \
